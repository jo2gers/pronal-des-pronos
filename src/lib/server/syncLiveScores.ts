...
export async function syncLiveScores(
	...
	opts: { force?: boolean } = {}
): Promise<{ ok: boolean; scanned: number; updated: number; ended: number; skipped?: string }> {
	const now = Date.now();
	...
	const { data: rawMatches } = await supabase
		.from('matches')
		.select('id, home_team, away_team, match_datetime, status, home_score, away_score, polymarket_event_slug')
	...
	for (const m of matches) {
		...
		if (isFinished && m.status !== 'finished') patch.status = 'finished';
	...
	return { ok: true, scanned: matches.length, updated, ended };
}
